...
class FeatureSet:
    ...
    def get_selected_feats(features: List[Feature], value=None, selector="models"):
        return [feature for feature in features if feature.is_in_attr_list(selector, value)] if features else []

    @staticmethod
    def get_feats_with_attr(features: List[Feature], attr="static"):
        return [feature for feature in features if feature.boolean_attr(attr)] if features else []
    ...
    def get_selected_feats_w_attr(self, value=None, selector="models", attr=None):
        return [feature for feature in self.features if feature.is_in_attr_list(selector, value) and
                feature.boolean_attr(attr)]

    def get_selected_feats_w_attrs(self, value=None, selector="models", attrs: List[str]=None):
        return [feat for feat in self.features if feat.is_in_attr_list(selector, value)
                and feat.boolean_attrs(attrs)]
